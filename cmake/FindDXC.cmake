...
if(_DXC_VENDOR_INCLUDE_DIR AND _DXC_VENDOR_LIBRARY)
    set(_DXC_VENDOR_HEADERS_COMPLETE TRUE)
    if(NOT WIN32 AND NOT EXISTS "${_DXC_VENDOR_INCLUDE_DIR}/dxc/WinAdapter.h")
        set(_DXC_VENDOR_HEADERS_COMPLETE FALSE)
    endif()
endif()

if(_DXC_VENDOR_INCLUDE_DIR AND _DXC_VENDOR_LIBRARY AND _DXC_VENDOR_HEADERS_COMPLETE)
...
if(NOT _DXC_RESOLVED_FROM_VENDOR)
    if(DEFINED DXC_INCLUDE_DIR AND DXC_INCLUDE_DIR MATCHES "^${DXC_VENDOR_ROOT}(/|$)")
        unset(DXC_INCLUDE_DIR CACHE)
    endif()
    if(DEFINED DXC_LIBRARY AND DXC_LIBRARY MATCHES "^${DXC_VENDOR_ROOT}(/|$)")
        unset(DXC_LIBRARY CACHE)
    endif()
    if(DEFINED DXC_IMPORT_LIBRARY AND DXC_IMPORT_LIBRARY MATCHES "^${DXC_VENDOR_ROOT}(/|$)")
        unset(DXC_IMPORT_LIBRARY CACHE)
    endif()
    if(DEFINED DXC_EXECUTABLE AND DXC_EXECUTABLE MATCHES "^${DXC_VENDOR_ROOT}(/|$)")
        unset(DXC_EXECUTABLE CACHE)
    endif()

    if(DEFINED _DXC_VENDOR_HEADERS_COMPLETE AND NOT _DXC_VENDOR_HEADERS_COMPLETE)
        list(REMOVE_ITEM DXC_SEARCH_PATHS ${DXC_VENDOR_PATHS})
    endif()
