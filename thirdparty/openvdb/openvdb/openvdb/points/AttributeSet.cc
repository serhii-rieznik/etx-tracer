...
template <typename T>
void eraseIndices(std::vector<T>& vec, const std::vector<size_t>& indices) {
...
AttributeSet::AttributeSet(const AttributeSet& attrSet, Index arrayLength, const AttributeArray::ScopedRegistryLock* lock)
  ...
  for (const auto& namePos : mDescr->map()) {
    ...
    if (hasMetadata)
      metadata = meta["default:" + namePos.first];
    ...
    AttributeArray::Ptr array = AttributeArray::create(mDescr->type(pos), arrayLength, stride, constantStride, metadata.get(), lock);

    // transfer hidden and transient flags
    if (existingArray->isHidden())
      array->setHidden(true);
    if (existingArray->isTransient())
...
AttributeSet::AttributeSet(const DescriptorPtr& descr, Index arrayLength, const AttributeArray::ScopedRegistryLock* lock)
  : mDescr(descr)
...
size_t AttributeSet::memUsage() const {
...
size_t AttributeSet::find(const std::string& name) const {
  return mDescr->find(name);
}

size_t AttributeSet::replace(const std::string& name, const AttributeArray::Ptr& attr) {
  const size_t pos = this->find(name);
  return pos != INVALID_POS ? this->replace(pos, attr) : INVALID_POS;
}

size_t AttributeSet::replace(size_t pos, const AttributeArray::Ptr& attr) {
...
const AttributeArray* AttributeSet::getConst(const std::string& name) const {
  const size_t pos = this->find(name);
  if (pos < mAttrs.size())
...
const AttributeArray* AttributeSet::get(const std::string& name) const {
  return this->getConst(name);
}

AttributeArray* AttributeSet::get(const std::string& name) {
  const size_t pos = this->find(name);
  if (pos < mAttrs.size())
    return this->get(pos);
  return nullptr;
...
const AttributeArray* AttributeSet::getConst(size_t pos) const {
  assert(pos != INVALID_POS);
  assert(pos < mAttrs.size());
  return mAttrs[pos].get();
}

const AttributeArray* AttributeSet::get(size_t pos) const {
  assert(pos != INVALID_POS);
  assert(pos < mAttrs.size());
  return this->getConst(pos);
}

AttributeArray* AttributeSet::get(size_t pos) {
  makeUnique(pos);
  return mAttrs[pos].get();
}

size_t AttributeSet::groupOffset(const Name& group) const {
  return mDescr->groupOffset(group);
}

size_t AttributeSet::groupOffset(const Util::GroupIndex& index) const {
  return mDescr->groupOffset(index);
}

AttributeSet::Descriptor::GroupIndex AttributeSet::groupIndex(const Name& group) const {
  return mDescr->groupIndex(group);
}

AttributeSet::Descriptor::GroupIndex AttributeSet::groupIndex(const size_t offset) const {
  return mDescr->groupIndex(offset);
}

std::vector<size_t> AttributeSet::groupAttributeIndices() const {
...
bool AttributeSet::isShared(size_t pos) const {
  assert(pos != INVALID_POS);
  assert(pos < mAttrs.size());
  return mAttrs[pos].use_count() > 1;
}

void AttributeSet::makeUnique(size_t pos) {
  assert(pos != INVALID_POS);
  assert(pos < mAttrs.size());
...
AttributeArray::Ptr AttributeSet::appendAttribute(const Name& name, const NamePair& type, const Index strideOrTotalSize, const bool constantStride, const Metadata* defaultValue) {
  Descriptor::Ptr descriptor = mDescr->duplicateAppend(name, type);

  // store the attribute default value in the descriptor metadata
  if (defaultValue)
    descriptor->setDefaultValue(name, *defaultValue);
...
AttributeArray::Ptr AttributeSet::appendAttribute(const Descriptor& expected, DescriptorPtr& replacement, const size_t pos, const Index strideOrTotalSize,
  const bool constantStride, const Metadata* defaultValue, const AttributeArray::ScopedRegistryLock* lock) {
  ...
  AttributeArray::Ptr array = AttributeArray::create(type, arrayLength, strideOrTotalSize, constantStride, defaultValue, lock);
...
AttributeArray::Ptr AttributeSet::removeAttribute(const Name& name) {
  const size_t pos = this->find(name);
  if (pos == INVALID_POS)
    return AttributeArray::Ptr();
  return this->removeAttribute(pos);
}

AttributeArray::Ptr AttributeSet::removeAttribute(const size_t pos) {
  if (pos >= mAttrs.size())
    return AttributeArray::Ptr();
...
AttributeArray::Ptr AttributeSet::removeAttributeUnsafe(const size_t pos) {
  if (pos >= mAttrs.size())
    return AttributeArray::Ptr();
...
void AttributeSet::dropAttributes(const std::vector<size_t>& pos) {
  if (pos.empty())
    return;
...
void AttributeSet::dropAttributes(const std::vector<size_t>& pos, const Descriptor& expected, DescriptorPtr& replacement) {
  if (pos.empty())
    return;
...
void AttributeSet::renameAttributes(const Descriptor& expected, const DescriptorPtr& replacement) {
  // ensure the descriptor is as expected
  if (*mDescr != expected) {
...
void AttributeSet::reorderAttributes(const DescriptorPtr& replacement) {
...
void AttributeSet::resetDescriptor(const DescriptorPtr& replacement, const bool allowMismatchingDescriptors) {
...
void AttributeSet::read(std::istream& is) {
  this->readDescriptor(is);
  this->readMetadata(is);
  this->readAttributes(is);
}

void AttributeSet::write(std::ostream& os, bool outputTransient) const {
  this->writeDescriptor(os, outputTransient);
  this->writeMetadata(os, outputTransient);
  this->writeAttributes(os, outputTransient);
}

void AttributeSet::readDescriptor(std::istream& is) {
  mDescr->read(is);
}

void AttributeSet::writeDescriptor(std::ostream& os, bool outputTransient) const {
...
void AttributeSet::readMetadata(std::istream& is) {
...
void AttributeSet::writeMetadata(std::ostream& os, bool outputTransient, bool paged) const {
...
void AttributeSet::readAttributes(std::istream& is) {
  for (size_t i = 0; i < mAttrs.size(); i++) {
    mAttrs[i]->readBuffers(is);
  }
}

void AttributeSet::writeAttributes(std::ostream& os, bool outputTransient) const {
...
bool AttributeSet::operator==(const AttributeSet& other) const {
  if (*this->mDescr != *other.mDescr)
    return false;
  if (this->mAttrs.size() != other.mAttrs.size())
    return false;

  for (size_t n = 0; n < this->mAttrs.size(); ++n) {
    if (*this->mAttrs[n] != *other.mAttrs[n])
      return false;
  }
  return true;
}
...
bool AttributeSet::Descriptor::operator==(const Descriptor& rhs) const {
  if (this == &rhs)
    return true;

  if (mTypes.size() != rhs.mTypes.size() || mNameMap.size() != rhs.mNameMap.size() || mGroupMap.size() != rhs.mGroupMap.size()) {
    return false;
  }

  for (size_t n = 0, N = mTypes.size(); n < N; ++n) {
    if (mTypes[n] != rhs.mTypes[n])
      return false;
  }

  if (this->mMetadata != rhs.mMetadata)
    return false;

  return std::equal(mGroupMap.begin(), mGroupMap.end(), rhs.mGroupMap.begin()) && std::equal(mNameMap.begin(), mNameMap.end(), rhs.mNameMap.begin());
}

bool AttributeSet::Descriptor::hasSameAttributes(const Descriptor& rhs) const {
  if (this == &rhs)
    return true;
  ...
  for (const auto& namePos : mNameMap) {
    ...
    if (mTypes[namePos.second] != rhs.mTypes[index])
      return false;
  }
...
size_t AttributeSet::Descriptor::count(const NamePair& matchType) const {
  return std::count(mTypes.begin(), mTypes.end(), matchType);
}

size_t AttributeSet::Descriptor::memUsage() const {
...
size_t AttributeSet::Descriptor::find(const std::string& name) const {
...
size_t AttributeSet::Descriptor::rename(const std::string& fromName, const std::string& toName) {
  if (!validName(toName))
    throw RuntimeError("Attribute name contains invalid characters - " + toName);
...
size_t AttributeSet::Descriptor::renameGroup(const std::string& fromName, const std::string& toName) {
  if (!validName(toName))
    throw RuntimeError("Group name contains invalid characters - " + toName);
...
const Name& AttributeSet::Descriptor::valueType(size_t pos) const {
  // pos is assumed to exist
  return this->type(pos).first;
}

const NamePair& AttributeSet::Descriptor::type(size_t pos) const {
...
MetaMap& AttributeSet::Descriptor::getMetadata() {
  return mMetadata;
}

const MetaMap& AttributeSet::Descriptor::getMetadata() const {
  return mMetadata;
}

bool AttributeSet::Descriptor::hasDefaultValue(const Name& name) const {
  std::stringstream ss;
  ss << "default:" << name;
...
void AttributeSet::Descriptor::setDefaultValue(const Name& name, const Metadata& defaultValue) {
...
void AttributeSet::Descriptor::removeDefaultValue(const Name& name) {
  std::stringstream ss;
  ss << "default:" << name;
...
void AttributeSet::Descriptor::pruneUnusedDefaultValues() {
...
size_t AttributeSet::Descriptor::insert(const std::string& name, const NamePair& typeName) {
  if (!validName(name))
    throw RuntimeError("Attribute name contains invalid characters - " + name);
  ...
  if (it != mNameMap.end()) {
    ...
    if (!AttributeArray::isRegistered(typeName)) {
      OPENVDB_THROW(KeyError, "Failed to insert '" << name << "' with unregistered attribute type '" << typeName.first << "_" << typeName.second);
    }
...
AttributeSet::Descriptor::Ptr AttributeSet::Descriptor::create(const NameAndTypeVec& attrs, const NameToPosMap& groupMap, const MetaMap& metadata) {
...
AttributeSet::Descriptor::Ptr AttributeSet::Descriptor::create(const NamePair& positionType) {
  auto descr = std::make_shared<Descriptor>();
  descr->insert("P", positionType);
  return descr;
}

AttributeSet::Descriptor::Ptr AttributeSet::Descriptor::duplicateAppend(const Name& name, const NamePair& type) const {
  Inserter attributes;
...
AttributeSet::Descriptor::Ptr AttributeSet::Descriptor::duplicateDrop(const std::vector<size_t>& pos) const {
  NameAndTypeVec vec;
  this->appendTo(vec);
...
void AttributeSet::Descriptor::appendTo(NameAndTypeVec& attrs) const {
...
bool AttributeSet::Descriptor::hasGroup(const Name& group) const {
  return mGroupMap.find(group) != mGroupMap.end();
}

void AttributeSet::Descriptor::setGroup(const Name& group, const size_t offset, const bool checkValidOffset) {
  if (!validName(group)) {
...
void AttributeSet::Descriptor::dropGroup(const Name& group) {
  mGroupMap.erase(group);
}

void AttributeSet::Descriptor::clearGroups() {
  mGroupMap.clear();
}

const Name AttributeSet::Descriptor::uniqueName(const Name& name) const {
  auto it = mNameMap.find(name);
  if (it == mNameMap.end())
    return name;
...
const Name AttributeSet::Descriptor::uniqueGroupName(const Name& name) const {
...
size_t AttributeSet::Descriptor::groupOffset(const Name& group) const {
...
size_t AttributeSet::Descriptor::groupOffset(const Util::GroupIndex& index) const {
  ...
  for (const auto& namePos : mNameMap) {
    if (namePos.second < index.first && mTypes[namePos.second] == GroupAttributeArray::attributeType()) {
      relativeIndex++;
    }
  }
...
AttributeSet::Descriptor::GroupIndex AttributeSet::Descriptor::groupIndex(const Name& group) const {
...
AttributeSet::Descriptor::GroupIndex AttributeSet::Descriptor::groupIndex(const size_t offset) const {
  ...
  return Util::GroupIndex(groups[offset / this->groupBits()], static_cast<uint8_t>(offset % this->groupBits()));
}

size_t AttributeSet::Descriptor::availableGroups() const {
  // the number of group attributes * number of bits per group

  const size_t groupAttributes = this->count(GroupAttributeArray::attributeType());

  return groupAttributes * this->groupBits();
}

size_t AttributeSet::Descriptor::unusedGroups() const {
...
bool AttributeSet::Descriptor::canCompactGroups() const {
  // can compact if more unused groups than in one group attribute array

  return this->unusedGroups() >= this->groupBits();
}

size_t AttributeSet::Descriptor::unusedGroupOffset(size_t hint) const {
  ...
  if (hint != std::numeric_limits<Index>::max() && hint < availableGroups() && std::find(indices.begin(), indices.end(), hint) == indices.end()) {
    return hint;
  }
...
bool AttributeSet::Descriptor::requiresGroupMove(Name& sourceName, size_t& sourceOffset, size_t& targetOffset) const {
...
bool AttributeSet::Descriptor::groupIndexCollision(const Descriptor& rhs) const {
...
bool AttributeSet::Descriptor::validName(const Name& name) {
  if (name.empty())
    return false;
  return std::find_if(name.begin(), name.end(), [&](int c) {
    return !(isalnum(c) || (c == '_') || (c == '|') || (c == ':'));
  }) == name.end();
}

void AttributeSet::Descriptor::parseNames(std::vector<std::string>& includeNames, std::vector<std::string>& excludeNames, bool& includeAll, const std::string& nameStr) {
  includeAll = false;
  ...
  while (tokenStream >> token) {
    ...
    if (negate) {
      if (token.length() < 2)
        throw RuntimeError("Negate character (^) must prefix a name.");
...
void AttributeSet::Descriptor::parseNames(std::vector<std::string>& includeNames, std::vector<std::string>& excludeNames, const std::string& nameStr) {
  bool includeAll = false;
  Descriptor::parseNames(includeNames, excludeNames, includeAll, nameStr);
}

void AttributeSet::Descriptor::write(std::ostream& os) const {
...
void AttributeSet::Descriptor::read(std::istream& is) {
  ...
  for (Index64 n = 0; n < arraylength; ++n) {
    ...
    if (!validName(nameAndOffset.first))
      throw IoError("Attribute name contains invalid characters - " + nameAndOffset.first);
  ...
  for (Index64 n = 0; n < grouplength; ++n) {
    nameAndOffset.first = readString(is);
    if (!validName(nameAndOffset.first))
      throw IoError("Group name contains invalid characters - " + nameAndOffset.first);
    is.read(reinterpret_cast<char*>(&nameAndOffset.second), sizeof(Index64));
