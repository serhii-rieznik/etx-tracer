...
RAYGEN(main) {
  ...
  if (bsdf::is_delta(mat, intersection.tex, global.scene, state.sampler) == false) {
    ...
    vcm_connect_to_light_path(global.scene, tri, mat, intersection, iteration, light_path, global.light_vertices, rt, state);
  }

  if (state.path_length + 1 <= kVCMMaxDepth) {
    state.merged += global.spatial_grid.gather(global.scene, state, global.light_vertices.a, intersection, kVCMMaxDepth, iteration.vc_weight);
  }
