...
inline double upbp_bpt_connection_cross_technique_weight(const UPBPBPTConnectionMISInput& input) {
  if ((input.scene == nullptr) || (input.light_vertex == nullptr) || (input.camera_vertex == nullptr) || (input.connection_segment == nullptr) ||
      (input.configuration.enabled(UPBPTechnique::BPT) == false)) {
    return 0.0;
  }
  ...
  const double light_local_factor = upbp_recursive_local_pde_factor(input.configuration, light_vertex, input.light_weights, 1.0 / (camera_to_light_transport * light_event_density),
    light_vertex.cls == UPBPVertexClass::Medium ? 1.0 / light_event_density : 0.0, light_sin_theta, PathSource::Light);
  const double camera_local_factor =
    upbp_recursive_local_pde_factor(input.configuration, camera_vertex, input.camera_weights, 1.0 / (light_to_camera_transport * camera_event_density),
      camera_vertex.cls == UPBPVertexClass::Medium ? 1.0 / camera_event_density : 0.0, camera_sin_theta, PathSource::Camera);
  ...
  const double w_light = camera_to_light_density * (light_local_factor + static_cast<double>(input.light_weights.previous_delta == false) * input.light_weights.d_shared +
                                                     input.light_scattering.pdf_reverse * input.light_weights.d_bpt / input.light_weights.ray_sample_reverse_pdf_inverse);
  const double w_camera = light_to_camera_density * (camera_local_factor + static_cast<double>(input.camera_weights.previous_delta == false) * input.camera_weights.d_shared +
                                                      input.camera_scattering.pdf_reverse * input.camera_weights.d_bpt / input.camera_weights.ray_sample_reverse_pdf_inverse);
  const double denominator = w_light + 1.0 + w_camera;
...
inline double upbp_bpt_direct_hit_cross_technique_weight(const UPBPPathRecord& camera_path, const uint32_t emitter_vertex_index, const UPBPRecursiveVertexWeights& camera_weights,
  const double emitter_selection_pdf, const double direct_area_pdf, const double emission_direction_pdf, const bool bpt_enabled) {
  if ((bpt_enabled == false) || (emitter_vertex_index == 0u) || (emitter_vertex_index >= camera_path.vertices.size()) || (emitter_selection_pdf <= 0.0) ||
      (direct_area_pdf <= 0.0) || (emission_direction_pdf <= 0.0)) {
    return 0.0;
  }
  ...
  const double w_camera = emitter_selection_pdf * direct_area_pdf * static_cast<double>(camera_weights.previous_delta == false) * camera_weights.d_shared +
                          emitter_selection_pdf * emission_direction_pdf * reverse_ray_pdf * camera_weights.d_bpt;
  const double denominator = 1.0 + w_camera;
  return (denominator > 0.0) && std::isfinite(denominator) ? 1.0 / denominator : 0.0;
}
...
inline double upbp_bpt_nee_cross_technique_weight(const UPBPBPTNEEMISInput& input) {
  ...
  const double local_factor = upbp_recursive_local_pde_factor(input.configuration, camera_vertex, input.camera_weights, 1.0 / reverse_ray_pdf,
    camera_vertex.cls == UPBPVertexClass::Medium ? 1.0 / camera_event_density : 0.0, sin_theta, PathSource::Camera);
  ...
  const double w_camera = emission_to_direct_ratio * reverse_ray_pdf *
                          (local_factor + static_cast<double>(input.camera_weights.previous_delta == false) * input.camera_weights.d_shared +
                            input.camera_scattering.pdf_reverse * input.camera_weights.d_bpt / input.camera_weights.ray_sample_reverse_pdf_inverse);
  const double denominator = w_light + 1.0 + w_camera;
  return (denominator > 0.0) && std::isfinite(denominator) ? 1.0 / denominator : 0.0;
}
...
inline double upbp_bpt_light_tracing_cross_technique_weight(const UPBPBPTLightTracingMISInput& input) {
  if ((input.scene == nullptr) || (input.light_vertex == nullptr) || (input.connection_segment == nullptr) || (input.light_subpath_count == 0u) ||
      (input.configuration.enabled(UPBPTechnique::BPT) == false) || (input.camera_sample.pdf_dir_out <= 0.0f)) {
    return 0.0;
  }
  const UPBPPathVertexRecord& light_vertex = *input.light_vertex;
  ...
  const double local_factor = upbp_recursive_local_pde_factor(input.configuration, light_vertex, input.light_weights, 1.0 / reverse_ray_pdf,
    light_vertex.cls == UPBPVertexClass::Medium ? 1.0 / light_event_density : 0.0, sin_theta, PathSource::Light);
  const double w_light = camera_area_density * reverse_ray_pdf *
                         (local_factor + static_cast<double>(input.light_weights.previous_delta == false) * input.light_weights.d_shared +
                           input.light_scattering.pdf_reverse * input.light_weights.d_bpt / input.light_weights.ray_sample_reverse_pdf_inverse);
  const double denominator = 1.0 + w_light;
  return (denominator > 0.0) && std::isfinite(denominator) ? 1.0 / denominator : 0.0;
}
