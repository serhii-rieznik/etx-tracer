...
void wavefront_store_hit(uint descriptor_index, uint index, GPUWavefrontHit hit) {
  ...
  buffer.Store(base_offset + kGPUWavefrontHitInstanceIndexOffset, hit.instance_index);
}
...
GPUWavefrontFastLightEndpoint wavefront_load_fast_light_endpoint(uint descriptor_index, uint path_index) {
  ByteAddressBuffer buffer = WAVEFRONT_RO_BUFFER(descriptor_index);
  const uint base_offset = path_index * kGPUWavefrontFastLightEndpointStride;
  GPUWavefrontFastLightEndpoint result = (GPUWavefrontFastLightEndpoint)0;
  result.emitter_pdf_from_prev = asfloat(buffer.Load(base_offset + kGPUWavefrontFastLightEndpointEmitterPdfFromPrevOffset));
  result.emitter_pdf_from_next = asfloat(buffer.Load(base_offset + kGPUWavefrontFastLightEndpointEmitterPdfFromNextOffset));
  result.emitter_flags = buffer.Load(base_offset + kGPUWavefrontFastLightEndpointEmitterFlagsOffset);
  result.first_vertex_flags = buffer.Load(base_offset + kGPUWavefrontFastLightEndpointFirstVertexFlagsOffset);
  return result;
}

void wavefront_store_fast_light_endpoint(uint descriptor_index, uint path_index, GPUWavefrontPathVertex emitter_root, GPUWavefrontPathVertex first_light_vertex) {
  RWByteAddressBuffer buffer = WAVEFRONT_RW_BUFFER(descriptor_index);
  const uint base_offset = path_index * kGPUWavefrontFastLightEndpointStride;
  buffer.Store(base_offset + kGPUWavefrontFastLightEndpointEmitterPdfFromPrevOffset, asuint(emitter_root.pdf_from_prev));
  buffer.Store(base_offset + kGPUWavefrontFastLightEndpointEmitterPdfFromNextOffset, asuint(emitter_root.pdf_from_next));
  buffer.Store(base_offset + kGPUWavefrontFastLightEndpointEmitterFlagsOffset, emitter_root.flags);
  buffer.Store(base_offset + kGPUWavefrontFastLightEndpointFirstVertexFlagsOffset, first_light_vertex.flags);
}
...
void wavefront_store_direct_light_sample(uint descriptor_index, uint index, GPUWavefrontDirectLightSample sample_value) {
  ...
  buffer.Store(base_offset + kGPUWavefrontDirectLightSamplePdfDirOutOffset, asuint(sample_value.pdf_dir_out));
}
...
GPUWavefrontDirectLightSample wavefront_load_direct_light_sample(uint descriptor_index, uint index) {
  ...
  sample_value.pdf_dir_out = asfloat(buffer.Load(base_offset + kGPUWavefrontDirectLightSamplePdfDirOutOffset));
  return sample_value;
}
...
void wavefront_store_connect_light_task(uint descriptor_index, uint index, GPUWavefrontConnectLightTask task) {
  ...
  buffer.Store(base_offset + kGPUWavefrontConnectLightTaskUPBPCameraPdfReverseBitsOffset, task.upbp_camera_pdf_reverse_bits);
  buffer.Store(base_offset + kGPUWavefrontConnectLightTaskUPBPLightPdfForwardBitsOffset, task.upbp_light_pdf_forward_bits);
...
GPUWavefrontConnectLightTask wavefront_load_connect_light_task(uint descriptor_index, uint index) {
  ...
  result_value.upbp_camera_pdf_reverse_bits = buffer.Load(base_offset + kGPUWavefrontConnectLightTaskUPBPCameraPdfReverseBitsOffset);
  result_value.upbp_light_pdf_forward_bits = buffer.Load(base_offset + kGPUWavefrontConnectLightTaskUPBPLightPdfForwardBitsOffset);
  result_value.upbp_light_pdf_reverse_bits = buffer.Load(base_offset + kGPUWavefrontConnectLightTaskUPBPLightPdfReverseBitsOffset);
  result_value.upbp_intersection_seed = buffer.Load(base_offset + kGPUWavefrontConnectLightTaskUPBPIntersectionSeedOffset);
  result_value.upbp_medium_seed = buffer.Load(base_offset + kGPUWavefrontConnectLightTaskUPBPMediumSeedOffset);
  return result_value;
}
...
void wavefront_store_connect_camera_task(uint descriptor_index, uint index, GPUWavefrontConnectCameraTask task) {
  ...
  buffer.Store(base_offset + kGPUWavefrontConnectCameraTaskUPBPScatteringPdfReverseBitsOffset, task.upbp_scattering_pdf_reverse_bits);
...
GPUWavefrontConnectCameraTask wavefront_load_connect_camera_task(uint descriptor_index, uint index) {
  ...
  result_value.upbp_scattering_pdf_reverse_bits = buffer.Load(base_offset + kGPUWavefrontConnectCameraTaskUPBPScatteringPdfReverseBitsOffset);
...
bool wavefront_hit_is_subsurface(GPUWavefrontHit hit) {
  return (hit.flags & GPUWavefrontHitFlags::Subsurface) != 0u;
}
...
bool wavefront_path_vertex_is_subsurface(GPUWavefrontPathVertex vertex) {
  return (vertex.flags & GPUWavefrontVertexFlags::Subsurface) != 0u;
}
...
uint wavefront_vertex_capacity(GPUWavefrontResources resources, bool from_camera) {
  return from_camera ? resources.camera_vertex_capacity : resources.light_vertex_capacity;
}
...
void wavefront_write_root_camera_vertex(uint path_index, Camera camera, Ray ray, SpectralQuery spect, uint pixel_index) {
  ...
  vertex.reserved0 = kInvalidIndex;
...
void wavefront_write_root_light_vertex(uint path_index, WavefrontEmitterSample emitter_sample, SpectralQuery spect, uint pixel_index) {
  ...
  vertex.instance_index = emitter_sample.instance_index;
  ...
  if (emitter_sample.is_distant == 0u) {
    ...
    if (try_load_emitter_instance(emitter_sample.emitter_index, emitter_instance) && (emitter_instance.triangle_index != kInvalidIndex)) {
      ...
      vertex.geo_normal = scene_instance_transform_geometric_normal(load_scene_instance(emitter_instance.instance_index), tri.geo_n);
      vertex.barycentric = emitter_sample.barycentric.yz;
    }
  }
  ...
  vertex.pdf_history = history_value;
  vertex.pdf_accumulated = history_value;
  uint vertex_slot = (resources.light_vertex_counter_buffer != kInvalidIndex) ? path_index : wavefront_light_vertex_slot(path_index, 0u);
  wavefront_store_path_vertex(resources.light_vertex_buffer, vertex_slot, vertex);
}

uint wavefront_light_vertex_append(GPUWavefrontResources resources) {
  RWByteAddressBuffer counter_buffer = bindless_rw_buffers[NonUniformResourceIndex(resources.light_vertex_counter_buffer)];
  uint vertex_slot = kInvalidIndex;
  counter_buffer.InterlockedAdd(0u, 1u, vertex_slot);
  return vertex_slot;
}
...
void wavefront_write_vertex(bool from_camera, uint path_index, inout GPUWavefrontPathState state, GPUWavefrontHit hit) {
  ...
  uint vertex_slot = from_camera ? wavefront_camera_vertex_slot(path_index, state.path_length)
                                 : ((resources.light_vertex_counter_buffer != kInvalidIndex) ? wavefront_light_vertex_append(resources)
                                                                                             : wavefront_light_vertex_slot(path_index, state.path_length));
  if (vertex_slot >= wavefront_vertex_capacity(resources, from_camera)) {
    return;
  }
  ...
  vertex.instance_index = hit.instance_index;
  vertex.texcoord = hit.vertex.tex;
  vertex.barycentric = hit.barycentric;
  ...
  vertex.reserved0 = from_camera ? kInvalidIndex : state.last_vertex_index;
  ...
  state.last_vertex_index = vertex_slot;
}

void wavefront_write_medium_vertex(bool from_camera, uint path_index, inout GPUWavefrontPathState state, GPUWavefrontHit hit) {
  GPUWavefrontResources resources = wavefront_load_resources();
  uint vertex_slot = from_camera ? wavefront_camera_vertex_slot(path_index, state.path_length)
                                 : ((resources.light_vertex_counter_buffer != kInvalidIndex) ? wavefront_light_vertex_append(resources)
                                                                                             : wavefront_light_vertex_slot(path_index, state.path_length));
  ...
  if (wavefront_hit_is_subsurface(hit)) {
    vertex.flags |= GPUWavefrontVertexFlags::Subsurface;
  }
...
# include "gpu_rt_wavefront_upbp_mis.hlsl"
# include "gpu_rt_wavefront_upbp_path.hlsl"
# include "gpu_rt_wavefront_upbp_medium.hlsl"
#endif
