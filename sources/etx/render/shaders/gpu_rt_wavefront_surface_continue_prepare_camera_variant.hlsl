...
BSDFSample wavefront_surface_continue_stage_bsdf_sample(
  BSDFResourceContext context, BSDFData data, Material material, inout Sampler sampler) {
  return ETX_STAGE_BSDF_SAMPLE(context, data, material, sampler);
}

float wavefront_surface_continue_stage_reverse_bsdf_pdf(
  BSDFResourceContext context, BSDFData input_data, float3 outgoing_direction, Material material, inout Sampler sampler) {
...
[numthreads(64, 1, 1)] void wavefront_camera_continue_prepare_diffuse_main(uint3 dtid : SV_DispatchThreadID) {
  wavefront_surface_continue_prepare_specialized(true, dtid.x);
}

[numthreads(64, 1, 1)] void wavefront_camera_continue_prepare_plastic_main(uint3 dtid : SV_DispatchThreadID) {
  wavefront_surface_continue_prepare_specialized(true, dtid.x);
}

[numthreads(64, 1, 1)] void wavefront_camera_continue_prepare_conductor_main(uint3 dtid : SV_DispatchThreadID) {
  wavefront_surface_continue_prepare_specialized(true, dtid.x);
}

[numthreads(64, 1, 1)] void wavefront_camera_continue_prepare_dielectric_main(uint3 dtid : SV_DispatchThreadID) {
  wavefront_surface_continue_prepare_specialized(true, dtid.x);
}

[numthreads(64, 1, 1)] void wavefront_camera_continue_prepare_thinfilm_main(uint3 dtid : SV_DispatchThreadID) {
  wavefront_surface_continue_prepare_specialized(true, dtid.x);
}
