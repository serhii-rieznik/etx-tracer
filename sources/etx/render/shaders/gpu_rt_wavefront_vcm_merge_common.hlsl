...
void wavefront_vcm_merge(uint dispatch_index) {
  ...
  [unroll] for (uint cell_offset = 0u; cell_offset < 8u; ++cell_offset) {
    ...
    while ((light_index != kInvalidIndex) && (light_index < constants.vcm_light_vertex_count) && (visited < resources.light_vertex_capacity)) {
      ...
      uint vcm_light_path_length = (light_vertex.path_length > 0u) ? (light_vertex.path_length - 1u) : 0u;
      if (wavefront_path_vertex_valid(light_vertex) && wavefront_path_vertex_is_surface(light_vertex) && wavefront_path_vertex_connectible(light_vertex) &&
