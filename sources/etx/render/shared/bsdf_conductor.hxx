...
ETX_GPU_CODE BSDFSample sample(const BSDFData& data, const Scene& scene, Sampler& smp) {
  ...
  return {eval_data.w_o, evaluate(eval_data, scene, smp), 0};
}
...
ETX_GPU_CODE BSDFSample sample(const BSDFData& data, const Scene& scene, Sampler& smp) {
  ...
  result.w_o = external::sample_conductor(data.spectrum_sample, smp, w_i, alpha_x, alpha_y,  //
    data.material.ext_ior(data.spectrum_sample), data.material.int_ior(data.spectrum_sample), result.weight);
  {
...
ETX_GPU_CODE BSDFEval evaluate(const BSDFData& data, const Scene& scene, Sampler& smp) {
  ...
  auto alpha_x = data.material.roughness.x;
  auto alpha_y = data.material.roughness.y;
  auto ext_ior = data.material.ext_ior(data.spectrum_sample);
  auto int_ior = data.material.int_ior(data.spectrum_sample);

  BSDFEval result;
  if (smp.next() > 0.5f) {
    result.bsdf = 2.0f * external::eval_conductor(data.spectrum_sample, smp, w_i, w_o, alpha_x, alpha_y, ext_ior, int_ior);
    ETX_VALIDATE(result.bsdf);
  } else {
    result.bsdf = 2.0f * external::eval_conductor(data.spectrum_sample, smp, w_o, w_i, alpha_x, alpha_y, ext_ior, int_ior) / w_i.z * w_o.z;
    ETX_VALIDATE(result.bsdf);
  }
  result.bsdf *= bsdf::apply_image(data.spectrum_sample, data.material.specular(data.spectrum_sample), data.material.specular_image_index, data.tex, scene);
  ETX_VALIDATE(result.bsdf);
