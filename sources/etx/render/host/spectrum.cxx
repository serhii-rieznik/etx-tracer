...
SpectralDistribution SpectralDistribution::rgb(float3 rgb, const rgb::SpectrumSet& spectrums) {
  ...
  constexpr float wavelengths[rgb::SampleCount] = {380.000000f, 390.967743f, 401.935486f, 412.903229f, 423.870972f, 434.838715f, 445.806458f, 456.774200f, 467.741943f, 478.709686f,
  ...
  float2 samples[rgb::SampleCount] = {};
  for (uint32_t i = 0; i < rgb::SampleCount; ++i) {
    float p = weights[0] * spectrums.values[0][i] +  //
...
constexpr float kRGBReflectionWhite[SampleCount] = {1.0618958571272863e+00f, 1.0615019980348779e+00f, 1.0614335379927147e+00f, 1.0622711654692485e+00f, 1.0622036218416742e+00f,
...
constexpr float kRGBReflectionCyan[SampleCount] = {1.0414628021426751e+00f, 1.0328661533771188e+00f, 1.0126146228964314e+00f, 1.0350460524836209e+00f, 1.0078661447098567e+00f,
...
constexpr float kRGBReflectionMagenta[SampleCount] = {9.9422138151236850e-01f, 9.8986937122975682e-01f, 9.8293658286116958e-01f, 9.9627868399859310e-01f, 1.0198955019000133e+00f,
...
constexpr float kRGBReflectionYellow[SampleCount] = {5.5740622924920873e-03f, -4.7982831631446787e-03f, -5.2536564298613798e-03f, -6.4571480044499710e-03f,
  -5.9693514658007013e-03f, -2.1836716037686721e-03f, 1.6781120601055327e-02f, 9.6096355429062641e-02f, 2.1217357081986446e-01f, 3.6169133290685068e-01f, 5.3961011543232529e-01f,
  7.4408810492171507e-01f, 9.2209571148394054e-01f, 1.0460304298411225e+00f, 1.0513824989063714e+00f, 1.0511991822135085e+00f, 1.0510530911991052e+00f, 1.0517397230360510e+00f,
  1.0516043086790485e+00f, 1.0511944032061460e+00f, 1.0511590325868068e+00f, 1.0516612465483031e+00f, 1.0514038526836869e+00f, 1.0515941029228475e+00f, 1.0511460436960840e+00f,
  1.0515123758830476e+00f, 1.0508871369510702e+00f, 1.0508923708102380e+00f, 1.0477492815668303e+00f, 1.0493272144017338e+00f, 1.0435963333422726e+00f, 1.0392280772051465e+00f};

constexpr float kRGBReflectionRed[SampleCount] = {1.6575604867086180e-01f, 1.1846442802747797e-01f, 1.2408293329637447e-01f, 1.1371272058349924e-01f, 7.8992434518899132e-02f,
...
constexpr float kRGBReflectionGreen[SampleCount] = {2.6494153587602255e-03f, -5.0175013429732242e-03f, -1.2547236272489583e-02f, -9.4554964308388671e-03f, -1.2526086181600525e-02f,
...
constexpr float kRGBReflectionBlue[SampleCount] = {9.9209771469720676e-01f, 9.8876426059369127e-01f, 9.9539040744505636e-01f, 9.9529317353008218e-01f, 9.9181447411633950e-01f,
...
constexpr inline const float power(uint32_t cls, uint32_t clr, uint32_t i) {
  constexpr const float* ptr[2][7] = {
    {kRGBReflectionRed, kRGBReflectionGreen, kRGBReflectionBlue, kRGBReflectionYellow, kRGBReflectionMagenta, kRGBReflectionCyan, kRGBReflectionWhite},
...
void init_spectrums(Spectrums& spectrums) {
  for (uint32_t i = 0; i < SampleCount; ++i) {
    for (uint32_t c = 0; c < Color::Count; ++c) {
      spectrums.rgb_reflection.values[c][i] = 0.9436005f * power(0u, c, i);
      spectrums.rgb_illuminant.values[c][i] = 0.9400712f * power(1u, c, i);
    }
  }
}
